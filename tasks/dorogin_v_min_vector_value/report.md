...
- Следующее найденное минимальное значение рассылается всем процессам с помощью `MPI_Bcast`, чтобы во всех экземплярах задачи (`Task`) `GetOutput()` содержал один и тот же результат.
...
if rank == 0:
  global_min = min_element(data.begin(), data.end())
...
Данный подход упрощает и сохраняет работуспособность реализации, он легко увеличивается до более сложных, больших схем (разбиение данных по процессам и использование `MPI_Reduce`), нам достаточно данной формы.
...
применяет глобальный поиск минимума исключительно на нулевом процессе, в последствии этого основная цель – показ интеграции алгоритма с инфраструктурой MPI и тестового окружения курса.
...
Тесты на производительность показывают что выбранный алгоритм с линейной сложностью без ошибок обрабатывает большие объемы данных, MPI‑версия интегрирована в общую инфраструктуру курса и может служить базой для дальнейших тестов с более сложными схемами распараллеливания.
